#!/usr/bin/env node
/**
 * WebSocket benchmark client — new Rust-core SDK.
 *
 * Connects to the mock server, subscribes, receives data messages, and
 * reports throughput / latency / memory metrics as a single JSON line on
 * stdout.
 *
 * Usage:
...
// ---------------------------------------------------------------------------
let received = 0;
let t0 = null;                       // set on first data message
const latencies = new Float64Array(500000);  // pre-allocated, max 500K
let latIdx = 0;
let maxSerial = -1;
...
ws.stock.on('message', (data) => {
  ...
  // Sentinel: server finished sending
  ...
  if (msg.event === 'data') {
    if (t0 === null) t0 = Date.now();  // first data message
    received++;
