 ...
 */
public class StockClient implements AutoCloseable, StockClientInterface {
  ...
  public StockClient(Pointer pointer) {
   ...
   */
  public StockClient(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("StockClient object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("StockClient call counter would overflow");
      }
    ...
    public void run() {
      if (pointer != null) {
        UniffiHelpers.uniffiRustCall(status -> {
          UniffiLib.INSTANCE.uniffi_marketdata_uniffi_fn_free_stockclient(pointer, status);
          return null;
        });
      }
    }
  ...
  Pointer uniffiClonePointer() {
    return UniffiHelpers.uniffiRustCall(status -> {
      ...
      return UniffiLib.INSTANCE.uniffi_marketdata_uniffi_fn_clone_stockclient(pointer, status);
    });
  }
    ...
    public StockIntradayClient intraday()  {
            try {
                return FfiConverterTypeStockIntradayClient.INSTANCE.lift(
    callWithPointer(it -> {
        try {
