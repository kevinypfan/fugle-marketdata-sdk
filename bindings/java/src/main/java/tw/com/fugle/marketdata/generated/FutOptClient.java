 ...
 */
public class FutOptClient implements AutoCloseable, FutOptClientInterface {
  ...
  public FutOptClient(Pointer pointer) {
   ...
   */
  public FutOptClient(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("FutOptClient object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("FutOptClient call counter would overflow");
      }
    ...
    public void run() {
      if (pointer != null) {
        UniffiHelpers.uniffiRustCall(status -> {
          UniffiLib.INSTANCE.uniffi_marketdata_uniffi_fn_free_futoptclient(pointer, status);
          return null;
        });
      }
    }
  ...
  Pointer uniffiClonePointer() {
    return UniffiHelpers.uniffiRustCall(status -> {
      ...
      return UniffiLib.INSTANCE.uniffi_marketdata_uniffi_fn_clone_futoptclient(pointer, status);
    });
  }
    ...
    public FutOptIntradayClient intraday()  {
            try {
                return FfiConverterTypeFutOptIntradayClient.INSTANCE.lift(
    callWithPointer(it -> {
        try {
    
            return
    UniffiHelpers.uniffiRustCall( _status -> {
        return UniffiLib.INSTANCE.uniffi_marketdata_uniffi_fn_method_futoptclient_intraday(
            it, _status);
    });
