 ...
 */
public class RestClient implements AutoCloseable, RestClientInterface {
  ...
  public RestClient(Pointer pointer) {
   ...
   */
  public RestClient(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("RestClient object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("RestClient call counter would overflow");
      }
    ...
    public void run() {
      if (pointer != null) {
        UniffiHelpers.uniffiRustCall(status -> {
          UniffiLib.INSTANCE.uniffi_marketdata_uniffi_fn_free_restclient(pointer, status);
          return null;
        });
      }
    }
  ...
  Pointer uniffiClonePointer() {
    return UniffiHelpers.uniffiRustCall(status -> {
      ...
      return UniffiLib.INSTANCE.uniffi_marketdata_uniffi_fn_clone_restclient(pointer, status);
    });
  }
    ...
    public FutOptClient futopt()  {
            try {
                return FfiConverterTypeFutOptClient.INSTANCE.lift(
    callWithPointer(it -> {
        try {
    
            return
    UniffiHelpers.uniffiRustCall( _status -> {
        return UniffiLib.INSTANCE.uniffi_marketdata_uniffi_fn_method_restclient_futopt(
            it, _status);
    });
    ...
    public StockClient stock()  {
            try {
                return FfiConverterTypeStockClient.INSTANCE.lift(
    callWithPointer(it -> {
        try {
    
            return
    UniffiHelpers.uniffiRustCall( _status -> {
        return UniffiLib.INSTANCE.uniffi_marketdata_uniffi_fn_method_restclient_stock(
            it, _status);
    });
