...
## Purpose
Defines the `core::testing::MockWsServer` contract — an in-process WebSocket server that speaks the same subscribe-ACK protocol as production `core::websocket::protocol`. Gated behind `features = ["test-utils"]` (off by default; pulls `tokio-comp` transitively because the mock is async-only). Replaces hand-rolled echo servers in monitor / py / js / uniffi test crates; drift between mock and production protocol is caught by `core/tests/mock_server_smoke.rs`.
## Requirements
